...
describe('Actor vitals reducer', () => {
  ...
  it('should update the hunger level', () => {
    expect(reducer({ hunger: 5 }, actionCreators.updateHunger(10))).toEqual({
      hunger: 15
    });
    expect(reducer({ hunger: 11 }, actionCreators.updateHunger(-1))).toEqual({
      hunger: 10
    });
    // never go below 0
  ...
  it('should update the thirst level', () => {
    expect(reducer({ thirst: 5 }, actionCreators.updateThirst(10))).toEqual({
      thirst: 15
    });
    expect(reducer({ thirst: 11 }, actionCreators.updateThirst(-1))).toEqual({
      thirst: 10
    });
    // never go below 0
  ...
  it('should update the fatigue level', () => {
    expect(reducer({ fatigue: 5 }, actionCreators.updateFatigue(10))).toEqual({
      fatigue: 15
    });
    expect(reducer({ fatigue: 11 }, actionCreators.updateFatigue(-1))).toEqual({
      fatigue: 10
    });
    // never go below 0
    expect(reducer({ fatigue: 5 }, actionCreators.updateFatigue(-10))).toEqual(
